...
# Adicionar a função pipeline_processamento_ortese ao módulo para ser acessível externamente
# (Isso é necessário porque app.py importa processamento_api como um módulo)
# O app.py já faz um importlib.util.spec_from_file_location, então esta função precisa estar disponível lá.
# Não é necessário fazer nada aqui, apenas garantir que a função esteja definida no escopo global.
